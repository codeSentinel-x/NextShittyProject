...
public class Simple_NPC : MonoBehaviour {
    ...
    void Awake() {
        ...
        Timer._onTick += (x) => {
            if (_isDead) return;
            if (_isSleeping) {
                if (x % 9 == 0) DecreesFood();
                if (x % 2 == 0) Sleep();
            } else {
                if (x % 5 == 0) DecreesFood();
                if (x % 15 == 0) DecreesSleep();
    ...
    private void WakeUp(string reason) {
        Log("waked up.", reason: reason, color: Color.blue);
        _isSleeping = false;
    }
    ...
    private void GoToSleep() {
        Log("fallen asleep", color: Color.cyan, reason: "Exhaustion");
        _isSleeping = true;
    }
    ...
    private void LookForFood() {
        if (_inventory.Count > 0) {
            ...
            Log($"restore {_inventory[0].addition} points of food", reason: $"Used {_inventory[0].name} from inventory", color: Color.green);
    ...
    private void Die(string reason) {
        Log("died", reason: reason, color: Color.red, lifetime: true);
        _isDead = true;
    }
    ...
    public void Log(string message, string reason = "", Color color = default, bool lifetime = false, bool bold = false) {
